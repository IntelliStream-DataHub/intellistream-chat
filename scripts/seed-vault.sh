#!/usr/bin/env bash
# Seed Radiance secrets into a running OpenBao / Vault.
#
# Usage:
...
#   KEYCLOAK_CLIENT_ID=radiance \
#   KEYCLOAK_CLIENT_SECRET='...' \
#   KEYCLOAK_ISSUER_URI=https://auth.example/realms/radiance \
#   ./scripts/seed-vault.sh
#
# Writes to KV-v2 path `secret/radiance` (mount: secret, key: radiance) — matches
# VaultEnvironmentPostProcessor's default `radiance.vault.path=radiance`.
#
...
: "${BAO_TOKEN:=radiance-dev-token}"

: "${RADIANCE_DB_USERNAME:=radiance}"
: "${RADIANCE_DB_PASSWORD:=radiance}"
: "${KEYCLOAK_CLIENT_ID:=radiance}"
: "${KEYCLOAK_CLIENT_SECRET:?KEYCLOAK_CLIENT_SECRET must be set (no safe default — pull from keycloak/realm.json)}"
: "${KEYCLOAK_ISSUER_URI:=http://192.168.100.98:8081/realms/radiance}"

# Export so the inline python below can read them via os.environ.
export BAO_ADDR BAO_TOKEN RADIANCE_DB_USERNAME RADIANCE_DB_PASSWORD \
...
print(json.dumps({"data": {
    "db.username":            os.environ["RADIANCE_DB_USERNAME"],
    "db.password":            os.environ["RADIANCE_DB_PASSWORD"],
...
echo "Seeding $BAO_ADDR/v1/secret/data/radiance ..."
HTTP_CODE=$(curl -sS -o /tmp/seed-vault-response.json -w "%{http_code}" \
  -X POST "$BAO_ADDR/v1/secret/data/radiance" \
...
echo "  export RADIANCE_VAULT_ENABLED=true"
echo "  export RADIANCE_VAULT_URI=$BAO_ADDR"
echo "  export RADIANCE_VAULT_TOKEN=$BAO_TOKEN"
