...
- [x] **N32 · `LuceneConfig` property name drifted from the docs** low — code reads `ichat.search.lucene-dir` (`LuceneConfig.java:30`) but AGENT.md and the `IntegrationTestApplication` javadoc document `chat.search.lucene-dir`; an operator following the docs sets an ignored property. → Fix the docs (all sibling props use `ichat.`).
