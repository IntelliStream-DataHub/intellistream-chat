...
| `ICHAT_LINK_PREVIEWS_ENABLED` | `true` | Link preview cards under messages. The rest of the group — `ICHAT_LINK_PREVIEWS_DIR` (`./data/link-previews`, the copied pictures), `_MAX_HTML_BYTES`, `_MAX_IMAGE_BYTES`, `_TIMEOUT`, `_REFRESH_AFTER`, `_RETRY_FAILED_AFTER`, `_RETENTION`, `_THREADS`, `_QUEUE_CAPACITY` — is documented in `application.yml` and the site docs. |
