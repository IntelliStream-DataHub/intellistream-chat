...
public class PollRestController {
    ...
    public PollDto castVote(@PathVariable Long pollId,
        ...
        if (!rateLimiter.tryAcquire(me.getUsername(), "poll-vote", 30, java.time.Duration.ofMinutes(1))) {
            throw new ai.intellistream.chat.security.RateLimitExceededException("vote rate exceeded");
        }
     ...
     */
    private Long requireMembership(Long pollId, ai.intellistream.chat.domain.User me) {
     ...
     */
    private Long requireVoteAccess(Long pollId, ai.intellistream.chat.domain.User me) {
        var channel = hostChannel(pollId);
        channelService.requireWriteAccess(channel, me);
        return channel.getId();
    }

    private ai.intellistream.chat.domain.Channel hostChannel(Long pollId) {
