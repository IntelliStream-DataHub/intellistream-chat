 ...
 * the test runner. The {@code dev} profile sets {@code radiance.dev-tools.enabled=true}
 * in {@code application-dev.properties}; operators can also flip the flag at runtime
 * via the env var without redeploying.
 */
@ControllerAdvice(basePackages = "ai.intellistream.radiance.web")
public class DevToolsModelAdvice {

    private final boolean enabled;

    public DevToolsModelAdvice(@Value("${radiance.dev-tools.enabled:false}") boolean enabled) {
