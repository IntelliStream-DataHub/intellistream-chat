 ...
 * {@code radiance.vault.enabled=true} (env {@code RADIANCE_VAULT_ENABLED}), this runs before
 ...
 * <ul>
 *   <li>{@code radiance.vault.enabled} — gate. Off → no-op.</li>
 *   <li>{@code radiance.vault.uri} — Vault/OpenBao base URL, e.g. {@code http://localhost:8200}.</li>
 *   <li>{@code radiance.vault.token} — token credential. AppRole / Kubernetes auth is a
 *       follow-up; tokens cover dev + most single-host deployments.</li>
 *   <li>{@code radiance.vault.path} — KV-v2 path. Default {@code radiance} maps to
 *       {@code secret/data/radiance}; pass {@code mymount/myapp/secrets} to override the
 *       mount.</li>
 * </ul>
 *
...
public class VaultEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    public static final String SOURCE_NAME = "radiance-vault";

    @Override
    public void postProcessEnvironment(ConfigurableEnvironment env, SpringApplication application) {
        if (!Boolean.parseBoolean(env.getProperty("radiance.vault.enabled", "false"))) {
            return;
        }
        var uri = env.getProperty("radiance.vault.uri");
        var token = env.getProperty("radiance.vault.token");
        var path = env.getProperty("radiance.vault.path", "radiance");
        if (uri == null || uri.isBlank() || token == null || token.isBlank()) {
            ...
            throw new IllegalStateException(
                    "radiance.vault.enabled=true but radiance.vault.uri or radiance.vault.token is missing");
        }
