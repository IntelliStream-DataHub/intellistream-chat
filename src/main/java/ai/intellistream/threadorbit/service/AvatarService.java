...
public class AvatarService {
    ...
    public AvatarService(UserRepository userRepository,
                         @Value("${threadorbit.avatars.dir:./data/avatars}") String storageDir) {
    ...
    private static byte[] readAllCapped(InputStream in, long cap) throws IOException {
        ...
        try (in) {
            ...
            while ((n = in.read(buf)) != -1) {
                total += n;
                if (total > cap) {
                    throw new ai.intellistream.threadorbit.security.UploadTooLargeException(cap);
                }
                out.write(buf, 0, n);
            }
        }
    ...
    static String sniffContentType(BufferedInputStream in, String declared) throws IOException {
        ...
        return ai.intellistream.threadorbit.attachments.AttachmentBytes.sniffContentType(in, declared);
    }
}
