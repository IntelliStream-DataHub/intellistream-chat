...
- **Search runs against an embedded Lucene index** at `./data/lucene` (`MessageIndexService`). Writes are pushed by `MessageService` (channels) and `ConversationService` (DMs/groups) after the surrounding JPA transaction commits, so the index never holds rolled-back data. On startup, `LuceneBootstrap` rebuilds from `messages` if the directory is empty and then reconciles `conversation_messages` in both directions; `CleanupTasks` runs the same two reconciles periodically (CLEAN-3). Queries shorter than 2 chars are treated as empty. The configurable property is `ichat.search.lucene-dir`.
...
- **Search authorization tiers.** `SearchService.searchChannel` requires the standard channel read rules. `searchConversation` requires membership — there is no public tier for a DM. `searchAccessible` (the global search box) spans the viewer's joined channels **and** the conversations they belong to, as one ranked list. `searchEverywhere` (HTTP: `GET /api/search?scope=all`) reads every channel, never any conversation, and is gated on Spring authority `ROLE_ADMIN` (Keycloak realm role `ichat-admin`).
- **The conversation ACL is enforced inside the Lucene query, never after it.** `MessageIndexService.searchAccessible` combines the viewer's channel and conversation ids into the query as a required `FILTER` clause, using `TermInSetQuery` (no `maxClauseCount` limit, so a viewer in thousands of conversations neither throws nor gets a silently truncated filter). Post-filtering an unrestricted search leaks through hit counts, ranking, highlighting and pagination — don't reintroduce it. `MessageIndexAclTest` and `ConversationSearchAccessIT` fail if the filter clause is removed.
